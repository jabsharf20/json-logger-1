...
public class JsonloggerOperations {
	...
	public void logger(
			...
			CorrelationInfo correlationInfo, ComponentLocation location, @Config JsonloggerConfiguration config,
			FlowListener flowListener, CompletionCallback<Void, Void> callback) {
		...
		if (isLogEnabled(loggerProcessor.getPriority().toString())) {
						...
						if (v != null) {
							try {
								...
								if (v.getClass().getCanonicalName()
									...
									// LOGGER.debug("TypedValue MediaType: " +
									// typedVal.getDataType().getMediaType());
									// LOGGER.debug("TypedValue Type: " +
									// typedVal.getDataType().getType().getCanonicalName());
									// LOGGER.debug("TypedValue Class: " +
									// typedVal.getValue().getClass().getCanonicalName());
			 ...
			 * LoggerProcessor ordering which is defined by the field sequence in
	...
	public void loggerScope(
			...
			ComponentLocation location, CorrelationInfo correlationInfo, FlowListener flowListener, Chain operations,
		...
		try {
			// Add cache entry for initial timestamp based on unique EventId
			initialTimestamp = configs.getConfig(configurationRef).getCachedTimerTimestamp(correlationInfo.getEventId(),
					initialTimestamp);
		} catch (Exception e) {
			LOGGER.error(
					"initialTimestamp could not be retrieved from the cache config. Defaulting to current System.currentTimeMillis()",
		...
		if (elapsed == 0) {
			...
			flowListener.onComplete(
					new TimerRemoverRunnable(correlationInfo.getEventId(), configs.getConfig(configurationRef)));
		} else {
			LOGGER.debug("flowListener already configured");
		}
	...
	private String getFormattedTimestamp(Long loggerTimestamp) {
		/*
		 * Define timestamp: - DateTime: Defaults to ISO format - TimeZone: Defaults to
		 * UTC. Refer to https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
		 * for valid timezones
		 */
